...
namespace Hmm.ServiceApi.Areas.AutomobileInfoService.Filters
{
    public static class FilterExtensions
    {
        ...
        public static void CreateLinks(this ApiGasLog log, ActionContext context, LinkGenerator linkGen, int autoId)
        {
            if (context == null || linkGen == null)
            {
                return;
            ...
            var links = new List<Link>
            {
                ...
                new()
                {
                    Title = "Automobile",
                    Rel = "get_automobile",
                    Href = linkGen.GetUriByRouteValues(context.HttpContext, "GetAutomobileById", new { id = autoId }),
                    Method = "Get"
                },
                new()
                {
                    Title = "AddGasLog",
                    Rel = "create_gasLog",
                    Href = linkGen.GetUriByRouteValues(context.HttpContext, "AddGasLog", new { autoId }),
                    Method = "POST"
                },
                new()
                {
                ...
                new()
                {
                    ...
                    Href = linkGen.GetUriByRouteValues(context.HttpContext, "AddHistoryGasLog", new { autoId }),
                    Method = "POST"
                }
            };

            if (log.Discounts != null && log.Discounts.Any())
            {
                links.AddRange(log.Discounts.Select(discount => new Link
                {
                    Title = "discount",
                    Rel = "get_discount",

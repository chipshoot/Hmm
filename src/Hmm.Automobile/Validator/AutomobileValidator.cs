...
namespace Hmm.Automobile.Validator
{
    public class AutomobileValidator : EntityValidatorBase<AutomobileInfo>
    {
        ...
        {
            RuleFor(a => a.AuthorId).MustAsync(async (id, cancellation) => await HasValidAuthor(id, cancellation)).WithMessage("Have valid default author for Automobile");
            ...
            RuleFor(a => a.VIN).NotEmpty().WithMessage("Have valid PIN of the automobile");
            RuleFor(a => a.Plate).NotEmpty().WithMessage("Have valid plate of the automobile");
            RuleFor(a => a.MeterReading).GreaterThanOrEqualTo(0).WithMessage("Have valid meter reading of the automobile");
        }
    }
}
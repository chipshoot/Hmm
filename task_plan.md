...
| **Picked images are always copied to the vault (never linked); storage is managed by downsize-on-copy, not a link/copy toggle** | Copy guarantees the photo survives anything the user does in Photos (delete / iCloud optimize / revoke); a `phasset` link is fragile + creates an unresolvable-at-Free→Paid debt. The "don't waste storage" concern is met by shrinking to a 2048px / JPEG q85 vault copy via native codecs (`flutter_image_compress` → injectable `ImageDownsizer`; default no-op, prod wires `NativeImageDownsizer`). Also transcodes HEIC→JPEG. `phasset`/`cloudFile` kinds stay parked (Phases 16/17) as a future warned, Phase-18-gated option, never the default. Only affects NEW picks — existing full-res vault photos aren't retro-downsized. `docs/attachments-design.md` §"Link vs. copy". | 2026-05-30 |
| **Apple-ecosystem (iPhone+iPad same iCloud) byte sync stays parked; if built, prefer iCloud Drive vault over `phasset`** | An iPhone+iPad on one Apple ID can move bytes via iCloud (Photos for `phasset`, or a vault in an iCloud Drive container) so cloudStorage photos appear on both without a paid tier — but only same-iCloud (never Windows/Android, which still need cloudApi). iCloud Drive vault keeps the copy-robustness + no Free→Paid debt; `phasset` is zero-dup but fragile + Phase-18-debted. Explored in `docs/apple-ecosystem-byte-sync.md`. Not implemented. | 2026-05-30 |

#!/usr/bin/env bash
# ============================================================
# deploy-api.sh — publish Hmm.ServiceApi locally and push it to the
# Oracle VPS as a native systemd service.
#
# Pairs with: scripts/setup-api-vps.sh (one-time VPS bootstrap)
#
# Usage:
#   ./deploy-api.sh --deploy            # publish + upload + restart
#   ./deploy-api.sh --deploy --no-publish   # skip publish, just upload
#   ./deploy-api.sh --publish           # build only (no upload)
#   ./deploy-api.sh --restart           # restart service on VPS
#   ./deploy-api.sh --status            # remote systemctl status
#   ./deploy-api.sh --logs              # follow remote journalctl
#   ./deploy-api.sh --ssh               # interactive SSH
#
# Override defaults via environment:
#   API_VPS_HOST    (default: 132.145.102.175)
#   API_VPS_USER    (default: ubuntu)
#   API_SSH_KEY     (default: ~/.ssh/20220830-2236.key)
#   API_REMOTE_DIR  (default: /opt/hmm-api)
#   API_SRC         (default: ../src/Hmm.ServiceApi relative to this script)
#   API_PUBLISH_OUT (default: $TMPDIR/hmm-api-publish)
# ============================================================
set -euo pipefail
...
REMOTE_DIR="${API_REMOTE_DIR:-/opt/hmm-api}"
REMOTE_STAGE="${API_REMOTE_STAGE:-/tmp/hmm-api-staging}"
APP_USER="${API_APP_USER:-hmm-api}"
SERVICE="${API_SERVICE:-hmm-api}"
SRC_DIR="${API_SRC:-$SCRIPT_DIR/../src/Hmm.ServiceApi}"
PUBLISH_OUT="${API_PUBLISH_OUT:-${TMPDIR:-/tmp}/hmm-api-publish}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'HELP'
deploy-api.sh — publish Hmm.ServiceApi locally and push it to the Oracle VPS
...
Actions:
  ...
  --restart                 restart hmm-api service on VPS
...
publish_local() {
  banner "Publishing Hmm.ServiceApi (Release, framework-dependent)"
  echo "  source: $SRC_DIR"
...
case "$ACTION" in
  ...
  deploy)
    ...
    upload_and_install
    banner "Done"
    echo "  Tail logs:    ./deploy-api.sh --logs"
    echo "  Verify OIDC:  curl -s https://api.homemademessage.com/.well-known/openid-configuration | jq .issuer"
    ;;

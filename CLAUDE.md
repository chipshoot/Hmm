...
- `/v1/receipts/extract` - receipt image/PDF → structured-JSON extraction (shop, date, odometer, classified line items, tax) via Claude vision for the client's `receipt_scan` feature. `POST` multipart (`file`); `415`/`413` on bad type/oversize. Implemented following the `IGeocodingService` precedent: `AnthropicSettings` + `IReceiptExtractionService` / `ClaudeReceiptExtractionService` (`Hmm.Utility.Services`) call the Anthropic Messages API over `HttpClient` (default **Haiku 4.5**, forced tool-use structured output — no SDK dependency), returning `ApiReceiptDraft` (line-item `type` is PascalCase `Labour`/`Part`/`Fee` to match the client `LineItemType`). **Requires `AnthropicSettings__ApiKey` in the environment** (never committed; set on the VPS) — without it the endpoint returns "not configured". Design/plan in the **hmm_console** repo: `docs/superpowers/specs/2026-07-01-receipt-scan-autofill-design.md`, `.../plans/2026-07-01-receipt-scan-autofill.md`.

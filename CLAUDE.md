...
- `/v1/receipts/extract` - receipt image/PDF → structured-JSON extraction (shop, date, odometer, classified line items, tax) via a **swappable AI engine** for the client's `receipt_scan` feature. `POST` multipart (`file`); `415`/`413` on bad type/oversize; optional `?engine=<name>` / `?purpose=<key>` to override the engine per request. Returns `ApiReceiptDraft` (line-item `type` is PascalCase `Labour`/`Part`/`Fee` to match the client `LineItemType`).
  - **AI engine abstraction** (`Hmm.Utility.Services`): `IReceiptExtractionProvider` (one impl per provider — today `AnthropicReceiptExtractionProvider`, calling the Anthropic Messages API over `HttpClient` with default **Haiku 4.5** + forced tool-use structured output, no SDK). A `ReceiptExtractionService` facade resolves *which* engine via `IAiEngineSelector` (precedence: explicit `?engine` > `?purpose` route > config default), guards vision, and delegates through `IReceiptExtractionProviderRegistry`. Adding a self-hosted/other provider = a new `IReceiptExtractionProvider` + one DI registration + an `AiEngines` entry — no other change.
  - **Config** section `AiEngines` in `appsettings.json`: `{ Default, Routes (purpose→engine), Engines[] { Name, Provider, Model, BaseUrl, SupportsVision, MaxTokens } }`. **API keys come from the environment**, e.g. `AiEngines__Engines__0__ApiKey` for the `claude` engine (never committed; set on the VPS). Without a key the endpoint returns "not configured". Design/plan: **hmm_console** `docs/superpowers/specs/2026-07-01-receipt-scan-autofill-design.md` + this repo's `docs/superpowers/{specs,plans}/2026-07-04-ai-engine-switch*.md`.

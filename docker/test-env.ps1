...
$overrideFile = Join-Path $ScriptDir ".compose.host-data.yml"
@"
# Auto-generated by test-env.ps1 - mounts SQLite databases on the host
services:
  hmm-idp:
    volumes:
...
Write-Info "Using host bind mounts (override: .compose.host-data.yml)"
...
Write-Host "    Database:      SQLite (host: $DataDir)" -ForegroundColor White
Write-Host ""
Write-Host "  SQLite files on host (open with DBeaver):" -ForegroundColor Cyan
Write-Host "    IDP DB:        $DataDir\hmm-idp.db" -ForegroundColor White
...
Write-Host "    DBeaver:   Open $DataDir\hmm.db or $DataDir\hmm-idp.db" -ForegroundColor DarkGray
Write-Host ""

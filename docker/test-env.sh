...
# Host-side data directory for SQLite databases
...
mkdir -p "$DATA_DIR/idp" "$DATA_DIR/api" "$DATA_DIR/seq"
ok "Data directory: $DATA_DIR"

# ── Export bind mount paths for compose override ──────────────────────
export HMM_IDP_DATA_DIR="$DATA_DIR/idp"
export HMM_API_DATA_DIR="$DATA_DIR/api"
export HMM_SEQ_DATA_DIR="$DATA_DIR/seq"
...
services:
  seq:
    volumes:
      - ${HMM_SEQ_DATA_DIR}:/data

  hmm-idp:
    volumes:
      - ${HMM_IDP_DATA_DIR}:/app/data

  hmm-api:
    volumes:
      - ${HMM_API_DATA_DIR}:/app/data
YAML
...
echo ""
echo -e "  ${CYAN}SQLite files on host:${NC}"
echo -e "    IDP DB:        ${WHITE}$DATA_DIR/idp/hmm-idp.db${NC}"

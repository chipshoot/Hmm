...
#
# Starts the full Hmm backend in Docker with SQLite databases
# mounted on the HOST filesystem for easy inspection/backup.
#
# Services started:
...
# Host-side data directory for SQLite databases (easy to inspect with DBeaver)
...
# Write a temporary compose override to use host bind mounts
# IDP and API databases share the same host folder (different filenames: hmm-idp.db vs hmm.db)
OVERRIDE_FILE="$SCRIPT_DIR/.compose.host-data.yml"
cat > "$OVERRIDE_FILE" <<'YAML'
# Auto-generated by test-env.sh - mounts SQLite databases on the host
services:
  hmm-idp:
    volumes:
...
info "Using host bind mounts (override: .compose.host-data.yml)"
...
step "Starting Scenario 2 - SQLite (all services in Docker)..."
...
echo -e "    Database:      ${WHITE}SQLite (host: $DATA_DIR)${NC}"
echo ""
echo -e "  ${CYAN}SQLite files on host (open with DBeaver):${NC}"
echo -e "    IDP DB:        ${WHITE}$DATA_DIR/hmm-idp.db${NC}"

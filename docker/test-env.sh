...
#   - Hmm API (REST service)        localhost:5010  (PostgreSQL inside container)
#
# Seeded test users:
...
# Legacy: previously held the API's SQLite file. Kept defined so existing
# --reset-db / --down callers don't trip on an unbound variable while the
# directory is still on disk from older runs.
...
if [ "$ACTION" = "down" ]; then
    ...
    ok "Test environment stopped and named volumes removed"
    echo ""
    info "IDP PostgreSQL data was in named volume idp-postgres-data (removed)."
    info "API PostgreSQL data was in named volume api-postgres-data (removed)."
    if [ -d "$DATA_DIR" ]; then
...
if [ "$RESET_DB" = true ]; then
    ...
    # Stop containers first so the embedded Postgres processes shut down
    # cleanly before their named volumes are removed.
    docker compose "${COMPOSE_FILES[@]}" down --volumes --remove-orphans 2>/dev/null || true

    ok "IDP + API PostgreSQL volumes removed (via --volumes flag above)"

    # Sweep up any legacy SQLite files left from older runs.
    if [ -d "$DATA_DIR" ] && ls "$DATA_DIR"/hmm.db* >/dev/null 2>&1; then
        rm -f "$DATA_DIR"/hmm.db "$DATA_DIR"/hmm.db-wal "$DATA_DIR"/hmm.db-shm
        ok "Deleted legacy API SQLite files from $DATA_DIR"
    fi
fi

# ── Start Services ────────────────────────────────────────────────────
# Both DBs are embedded in their respective containers and persisted via
# named volumes (idp-postgres-data, api-postgres-data). No host bind mounts
# needed for the API any more.
...
info "Compose: compose.base-sqlite.yml + compose.idp.yml + compose.api.yml"
info "IDP: PostgreSQL (embedded)  |  API: PostgreSQL (embedded)"
echo ""
...
echo -e "    Mailpit:       ${WHITE}http://localhost:${MAILPIT_UI_PORT:-8025}${NC}  (captured verification / password-reset emails)"
echo ""
echo -e "  ${CYAN}Databases:${NC}"
echo -e "    IDP:           ${WHITE}PostgreSQL (embedded in hmm-idp container, volume: idp-postgres-data)${NC}"
echo -e "    API:           ${WHITE}PostgreSQL (embedded in hmm-api container, volume: api-postgres-data)${NC}"
echo ""
echo -e "  ${CYAN}IDP PostgreSQL (connect via docker exec):${NC}"
...
echo ""
echo -e "  ${CYAN}API PostgreSQL (connect via docker exec):${NC}"
echo -e "    Container:     ${WHITE}hmm-api${NC}"
echo -e "    Database:      ${WHITE}HmmNotes${NC}"
echo -e "    User:          ${WHITE}postgres${NC}"
echo -e "    Connect:       ${WHITE}docker exec -it hmm-api su postgres -c \"psql -h 127.0.0.1 -d HmmNotes\"${NC}"
echo ""
